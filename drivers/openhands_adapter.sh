...
OPENHANDS_RUNTIME_VALUE="${OPENHANDS_RUNTIME:-ai-lab}"
if [[ "${OPENHANDS_DRY_RUN:-0}" == "1" && -z "${OPENHANDS_RUNTIME:-}" ]]; then
  OPENHANDS_RUNTIME_VALUE="host"
fi
...
OPENHANDS_RUNTIME="${OPENHANDS_RUNTIME_VALUE}" \
bash "${REPO_ROOT}/scripts/openhands_start.sh" "${PROMPT}"
OPENHANDS_EXIT_CODE=$?
set -e

if [[ "${OPENHANDS_DRY_RUN:-0}" == "1" && ${OPENHANDS_EXIT_CODE} -eq 0 ]]; then
  "${PY_BIN}" - "${AEP_WORKSPACE}" "${ALLOWED_PATHS_RAW}" "${TASK}" <<'PY'
import json
import sys
from pathlib import Path
...
target.write_text(
    '"""Autogenerated by OpenHands dry-run adapter."""\n\n'
    "def run() -> dict[str, str]:\n"
    f'    return {{"task": "{task}", "status": "mocked"}}\n',
    encoding="utf-8",
)
PY
fi

...
def test_routing_selects_configured_default_agent(tmp_path: Path) -> None:
    ...
    resolver = RoutingResolver(manifests_dir, default_agent_id="primary")
    ...
    assert "default_agent_id=primary" in decision.rationale
...
def test_routing_requested_agent_overrides_default(tmp_path: Path) -> None:
    manifests_dir = tmp_path / "configs" / "agents"
    _write_manifest(manifests_dir, "primary", capabilities=["write_repo"])
    _write_manifest(
        manifests_dir,
...
def test_routing_policy_overlay_cannot_relax_hard_policy(tmp_path: Path) -> None:
    ...
    assert effective.max_patch_lines == 2000

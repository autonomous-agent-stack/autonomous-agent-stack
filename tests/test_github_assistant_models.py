"""Tests for github_assistant.models validation and serialization."""
...
_MODELS_PATH = Path(__file__).resolve().parents[1] / "src/autoresearch/github_assistant/models.py"
_SPEC = importlib.util.spec_from_file_location("github_assistant_models_under_test", _MODELS_PATH)
...
AssistantPolicy = _MODELS.AssistantPolicy
GitHubAssistantProfileConfig = _MODELS.GitHubAssistantProfileConfig
GitHubAssistantProfilesConfig = _MODELS.GitHubAssistantProfilesConfig
GitHubIssue = _MODELS.GitHubIssue
GitHubIssueComment = _MODELS.GitHubIssueComment
GitHubMergedPullRequest = _MODELS.GitHubMergedPullRequest
GitHubPullRequest = _MODELS.GitHubPullRequest
GitHubPullRequestFile = _MODELS.GitHubPullRequestFile
ManagedRepoConfig = _MODELS.ManagedRepoConfig
RepoCatalog = _MODELS.RepoCatalog
TriageIssueType = _MODELS.TriageIssueType
TriagePriority = _MODELS.TriagePriority
TriageResult = _MODELS.TriageResult
YouTubeIngestRouteConfig = _MODELS.YouTubeIngestRouteConfig
...
class TestYouTubeIngestRouteConfig:
    ...
    def test_normalize_blank_string_to_empty_list(self) -> None:
...
class TestManagedRepoConfig:
    ...
    def test_normalize_labels_map_string_values(self) -> None:
        cfg = ManagedRepoConfig(repo="owner/repo", labels_map={"feature": "  enhancement  "})
...
class TestGitHubModels:
    def test_github_issue_comment(self) -> None:
        comment = GitHubIssueComment(author="alice", body="Looks good")
    ...
    def test_github_pr_file(self) -> None:
        pr_file = GitHubPullRequestFile(path="src/main.py", additions=10, deletions=2)
        assert pr_file.path == "src/main.py"
    ...
    def test_github_merged_pr(self) -> None:
        merged_pr = GitHubMergedPullRequest(
            repo="owner/repo",
            number=3,
            title="Merge",

...
from content_kb.topic_classifier import classify_by_keywords, Topic
from content_kb.contracts import TopicClassification


class TestTopicClassifier:
    def test_ai_keywords(self) -> None:
        result = classify_by_keywords("讨论AI和大模型的最新发展")
        assert result.primary_topic == Topic.AI_STATUS

    def test_coding_keywords(self) -> None:
        result = classify_by_keywords("vibe coding 工作流分享")
        assert result.primary_topic == Topic.VIBE_CODING

    def test_economy_keywords(self) -> None:
        result = classify_by_keywords("股市投资策略分析")
        assert result.primary_topic == Topic.ECONOMY

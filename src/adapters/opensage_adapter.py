...
class StructuredLogger:
    ...
    def _log_json(self, level: str, data: Dict[str, Any]) -> None:
        """记录 JSON 格式日志"""
        log_entry = {
...
# ============================================
# AppleDouble 清理器（核心功能）
# ============================================

class AppleDoubleCleaner:
    """AppleDouble 文件清理器
    
    专门用于清理 macOS 产生的元数据文件：
    - ._ 开头的文件（AppleDouble 格式）
    - .DS_Store 文件
    """
    
    def __init__(self):
        self.logger = StructuredLogger("AppleDoubleCleaner")
    
    def clean_list(self, items: List[Any]) -> tuple[List[Any], Dict[str, Any]]:
        """清理列表中的 AppleDouble 文件引用
        ...
        for item in items:
            ...
            # 检测 AppleDouble 文件模式
            if self._is_appledouble(item_str):
                files_removed.append(item_str)
            else:
        ...
        # 记录清理日志（重点）
        self.logger.log_appledouble_cleanup(
            files_removed=len(files_removed),
            duration_ms=duration_ms,
            files=files_removed if files_removed else None
        )
        
        stats = {
            "total_items": len(items),
            "files_removed": len(files_removed),
            "files_kept": len(cleaned_items),
            "duration_ms": duration_ms,
            "removed_files": files_removed[:10]  # 只保留前10个
        }
        
        return cleaned_items, stats
    
    def clean_dict(self, data: Dict[str, Any]) -> tuple[Dict[str, Any], Dict[str, Any]]:
        """清理字典中的 AppleDouble 键
        ...
        # 记录清理日志（重点）
        self.logger.log_appledouble_cleanup(
            files_removed=len(keys_removed),
            duration_ms=duration_ms,
            files=keys_removed if keys_removed else None
        )
        
        stats = {
            "total_keys": len(data),
            "keys_removed": len(keys_removed),
            "keys_kept": len(cleaned_data),
            "duration_ms": duration_ms,
            "removed_keys": keys_removed[:10]
        }
        
        return cleaned_data, stats
    
    def clean_directory(self, directory: str) -> Dict[str, Any]:
        """清理目录中的 AppleDouble 文件
        ...
        try:
                ...
                for file in files:
                    if self._is_appledouble(file):
                        ...
                        try:
                            ...
                            self.logger.error(
                                node="executor",
                                action="appledouble_remove_failed",
                                file=file_path,
                                error=str(e)
                            )
        except Exception as e:
            self.logger.error(
                node="executor",
                action="appledouble_scan_failed",
                directory=directory,
                error=str(e)
            )
        ...
        return {
            ...
            "files_removed": len(files_removed),
            "duration_ms": duration_ms,
            "removed_files": files_removed[:10]
        }
    
    @staticmethod
    def _is_appledouble(name: str) -> bool:
        """检测是否为 AppleDouble 文件
...
class AdapterRegistry:
    ...
    def __init__(self, registry_path: str = None):
        ...
        self.logger = StructuredLogger("AdapterRegistry")
        self._load_registry()
...
class CleaningScriptGenerator:
    ...
    def __init__(self, output_dir: str = None):
        ...
        self.logger = StructuredLogger("CleaningScriptGenerator")
...
class DockerValidator:
    ...
    def __init__(self, image: str = "python:3.11-slim"):
        ...
        self.logger = StructuredLogger("DockerValidator")
...
class OpenSageAdapter:
    ...
    def __init__(self):
        ...
        self.logger = StructuredLogger("OpenSageAdapter")
    ...
    def clean_appledouble(self, data: Any) -> Any:
        """清理 AppleDouble 文件引用（公开接口）
        ...
        """
        if isinstance(data, list):
            cleaned, stats = self.cleaner.clean_list(data)
            return cleaned
        elif isinstance(data, dict):
            cleaned, stats = self.cleaner.clean_dict(data)
            return cleaned
        return data
    ...
    def _handle_parse_error(self, error: ParseError) -> Any:
        ...
        if existing_adapter:
            self.logger.info(
                action="using_existing_adapter",
                adapter_path=existing_adapter,

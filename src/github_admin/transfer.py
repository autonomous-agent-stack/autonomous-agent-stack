...
def render_transfer_plan_markdown(
    *,
    ...
    skipped = [decision for decision in decisions if decision.action == "skip"]
    ...
    lines = [
        ...
        "",
        "## Summary",
        f"- Planned transfers: {len(planned)}",
        f"- Manual review: {len(review)}",
        f"- Skipped: {len(skipped)}",
        "",
    ]
    lines.extend(_render_decision_group("Planned Transfers", planned))
    lines.extend(_render_decision_group("Manual Review", review))
    lines.extend(_render_decision_group("Skipped", skipped))
    return "\n".join(lines).rstrip() + "\n"


def _render_decision_group(title: str, decisions: list[GitHubAdminPlanDecisionRead]) -> list[str]:
    lines = [f"## {title}"]
    if not decisions:
        lines.extend(["- None", ""])
        return lines

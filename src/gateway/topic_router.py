...
class TopicRouter:
    ...
    async def route_message(
        self, message_type: str, text: str, mirror: bool = False, sender_id: Optional[str] = None
    ) -> Dict:
        """
        路由消息到指定话题
        ...
        try:
            ...
            if mirror:
                ...
                if backup_route:
                    logger.info(
                        "[Router-Gate] Mirror enabled, backing up to: %s", backup_route["thread_id"]
                    )
                    ...
                    mirror_result = await self.mirror.mirror_to_backup(
                        original_message, backup_route["chat_id"], backup_route["thread_id"]
                    )
            ...
            return {"status": "error", "error": str(e), "message_type": message_type}
    ...
    async def mirror_message(
        ...
        try:
            ...
            result = await self.mirror.mirror_to_backup(
                original_message, target_chat_id, target_thread_id
            )
    ...
    async def _send_message(self, chat_id: int, thread_id: Optional[int], text: str) -> int:
        """
        发送消息到指定话题
    ...
    async def broadcast_message(self, text: str, message_types: list = None) -> Dict:
        """
        广播消息到多个话题
        ...
        logger.info("[Router-Gate] Broadcast complete: %d/%d sent", sent_count, len(message_types))
    ...
    def add_route(
        self, message_type: str, chat_id: int, thread_id: Optional[int], description: str = ""
    ) -> bool:
        """
        添加新路由（代理方法）

...
class GitHubAdminService:
    ...
    def inventory(self, request: GitHubAdminInventoryRequest) -> GitHubAdminRunRead:
        ...
        preflight = GitHubAdminPreflightRead()
    ...
    def _build_summary(
        ...
        *,
        ...
        candidate_repo_count = sum(
            ...
            if not repository.suggested_exclude or any("heuristic:" in item or item.startswith("fork repo:") for item in repository.suggested_exclude_reasons)
        )
        ready_to_execute_count = sum(1 for item in preflight.repositories if item.readiness.value == "ready")
        blocked_count = sum(1 for item in preflight.repositories if item.readiness.value == "blocked")
        unknown_count = sum(1 for item in preflight.repositories if item.readiness.value == "unknown")

...
class BusinessTDD_Enforcer:
    ...
    def __init__(
        ...
        # 玛露必需关键词
        self.malu_keywords = ["挑战游泳级别持妆", "不用调色", "遮瑕力强"]
    ...
    async def validate_malu_copy(self, generated_copy: str) -> ValidationResult:
        ...
        result = ValidationResult(
            valid=True, missing_keywords=[], forbidden_found=[], test_passed=True, test_details={}
        )
    ...
    async def _semantic_check(self, text: str) -> Dict[str, Any]:
        ...
        semantic_ok = (
            len(required_hits) == len(self.malu_keywords) and not forbidden_hits and score >= 0.7
        )

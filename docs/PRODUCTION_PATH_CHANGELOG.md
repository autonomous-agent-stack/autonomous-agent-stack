...
## Changelog: Post-Run Gate Action → Approval Resolution

### What changed

When a post-run gated task is already in `APPROVAL_REQUIRED` (because gate_action
routed it there), approving it now reads `gate_action` from the persisted
`result_payload["gate_evaluation"]` to decide the next state, instead of always
re-executing.

### Before

```text
APPROVAL_REQUIRED task receives approve()
  → all cases: QUEUED → _execute() (re-run, no gate_action distinction)
  → needs_review / fallback / retry all treated identically
```
...
```text
APPROVAL_REQUIRED task receives approve()
  → read result_payload["gate_evaluation"]["gate_action"]
  → if gate_action == "needs_review" → COMPLETED   (human reviewed, accepted)
  → if gate_action == "fallback"     → QUEUED → _execute()  (re-run via fallback)
  → if reject                        → REJECTED
```

### Not changed

- Automatic retry: `gate_action == "retry"` does NOT auto-re-dispatch — requires manual approval
- Automatic fallback dispatch: no automatic agent switching
- `run_record` consumption: RunRecord remains write-only (stored, not read by downstream decisions)
- `result_payload["gate_evaluation"]` write path unchanged
- Non-gated tasks (direct execution without approval) unaffected

### Files changed

| File | Change |
|------|--------|
| `src/autoresearch/core/services/control_plane_service.py` | `approve_task()` reads gate_action and routes accordingly |

### Verification

```bash
PYTHONPATH=src pytest tests/test_linux_gate_integration.py -v
PYTHONPATH=src pytest tests/test_linux_gate_approval_resolution.py -v
```

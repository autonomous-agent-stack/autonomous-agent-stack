...
当智能体遇到未知阻碍时，**优先自动检索并分析寻找安全、成熟的开源库**（如通过 GitHub 筛选高星评项目），**若无合适可用方案**，再自主编写 Python 脚本。无论是引入第三方开源依赖还是自主手写代码，均需在安全沙盒中完成严格试错后，将其注册为全新的 MCP 工具。

**工作流程**：
1. **捕获异常**：检测到格式解析报错
2. **开源库检索**：GitHub 搜索 + 高星评项目筛选
3. **安全评估**：检查依赖安全性、许可证、维护状态
4. **自动触发**：生成专用清洗节点（开源库或自写代码）
5. **沙盒验证**：在 Docker 中执行严格试错
6. **工具注册**：成功后注入工具池
7. **热更新**：无需重启主服务

**优先级策略**：
- **第一优先**：开源库（GitHub Stars > 100, 最近更新 < 1 年）
- **第二优先**：自写代码（仅当无合适开源方案时）

**安全要求**：
- ✅ **开源库**：必须通过安全扫描（依赖漏洞检查）
- ✅ **自写代码**：必须在 Docker 沙盒中完成试错
- ✅ **统一注册**：无论来源，均需注入工具池

**实现文件**：
- `src/autoresearch/core/services/dynamic_tool_synthesis.py`
- `src/autoresearch/agents/opensource_searcher.py`（新增）

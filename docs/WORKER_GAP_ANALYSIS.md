...
| GateVerdict | `make_gate_verdict()` | ✅ 同上 | ✅ 同上 | ⚠️ post-run approval resolve 消费 gate_action |
...
- `gate_evaluation` 中的 verdict 不驱动 automatic retry/fallback dispatch — 但 `gate_action` 在 post-run approval resolve 时被消费（needs_review → COMPLETED, fallback → QUEUED → re-execute）
- `RunRecord` 仍不被 downstream 消费 — 仅存储

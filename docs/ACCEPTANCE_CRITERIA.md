...
| G7.13 | Post-run gated task approve 时，gate_action == `needs_review` → task COMPLETED | ✅ `pytest tests/test_housekeeper.py::test_housekeeper_post_run_needs_review_approval_completes_without_rerun` |
| G7.14 | Post-run gated task approve 时，gate_action == `fallback` → task QUEUED → `_execute()` | ✅ `pytest tests/test_housekeeper.py::test_housekeeper_post_run_fallback_approval_reruns_linux_path` |
| G7.15 | Post-run gated task reject → task REJECTED | ✅ `pytest tests/test_housekeeper.py::test_housekeeper_post_run_gated_reject_stays_rejected_without_rerun` |
| G7.16 | Non-gated task approve 不受 gate_action 影响，行为不变 | ✅ `pytest tests/test_housekeeper.py::test_housekeeper_api_creates_draft_then_control_plane_task_for_software_change` (existing coverage — non-gated approve path unchanged) |
